    ...
    override fun retrieveSignableData(): List<SignableDataResult> {
        return when (requestType) {
            is ApprovalRequestDetailsV2.Login -> {
                val dataToSend = requestType.jwtToken.toByteArray(charset = Charsets.UTF_8)
                listOf(SignableDataResult.Device(dataToSend, dataToSend))
            }

            is ApprovalRequestDetailsV2.VaultInvitation -> {
                val dataToSend = requestType.vaultName.toByteArray(charset = Charsets.UTF_8)
                listOf(SignableDataResult.Device(dataToSend, dataToSend))
            }

            is ApprovalRequestDetailsV2.PasswordReset -> {
                val dataToSend = requestId.toByteArray(charset = Charsets.UTF_8)
                listOf(SignableDataResult.Device(dataToSend, dataToSend))
            }
